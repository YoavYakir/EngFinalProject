...
def run_test(test_name, test_function, workers=0, batch_size=0, filter_size=0, sample_rate=0, epochs=0, learning_rate=0, model_size=""):
    ...
    monitor.save_results(system_stats, f"{test_name}", f"./results/cpu/clean_{date_string}.json")
...
def run_all_tests():
    ...
    ResourceMonitor.init_results_file(f"./results/cpu/clean_{date_string}.json")
