...
def run_test_with_nsys(test_name, batch_size, epochs, learning_rate, model_size, data_type, workers):
    ...
    results_file = f"./results/mnist/clean.json"
...
def run_test(test_name, test_function, batch_size=0, epochs=0, learning_rate=0, model_size="", data_type=None, workers=1):
    ...
    monitor.save_results(system_stats, f"{test_name}", f"./results/mnist/clean.json")
...
if __name__ == "__main__":
    ResourceMonitor.init_results_file(f"./results/mnist/clean.json")
    ...
    ResourceMonitor.fix_json_file(f"./results/mnist/clean.json")
